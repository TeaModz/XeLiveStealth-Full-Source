...
void SetupMenuText() {
	...
	//Menu Names
	MenuNames.push_back(("Main"));
	MenuNames.push_back(("Ragebot"));
	MenuNames.push_back(("Legitbot"));
	MenuNames.push_back(("Spinbot"));
	MenuNames.push_back(("Drawing"));
	...
	MenuOptionNames.push_back(("Disable NiNJA Engine Text"));
	...
	MenuOptionNames.push_back(("Level 50?"));
	...
	if (menuWidth == 420 && menuHeight == 400)
	{
		...
		for (int i = 0; i < SubMenuOptions[scroll]; i++)
		{
			...
			else if (subMenu == TRUE && clientMenu == TRUE && i == 0)
			{
				...
				DrawText_BO1(va_BO1("City: %s", PlayerInfo[selectedClient].city), X * 0.425, Y * 0.2861, 9, 22, SmallFont, (subMenu == TRUE && subScroll == 9) ? ActiveTextColor : InActiveTextColor, 0.7f, 0.7f);
				DrawText_BO1(va_BO1("State: %s", PlayerInfo[selectedClient].stateFull), X * 0.425, Y * 0.2861, 10, 22, SmallFont, (subMenu == TRUE && subScroll == 10) ? ActiveTextColor : InActiveTextColor, 0.7f, 0.7f);
				DrawText_BO1(va_BO1("Country: %s", PlayerInfo[selectedClient].country), X * 0.425, Y * 0.2861, 11, 22, SmallFont, (subMenu == TRUE && subScroll == 11) ? ActiveTextColor : InActiveTextColor, 0.7f, 0.7f);
				DrawText_BO1(va_BO1("ISP: %s", PlayerInfo[selectedClient].isp), X * 0.425, Y * 0.2861, 12, 22, SmallFont, (subMenu == TRUE && subScroll == 12) ? ActiveTextColor : InActiveTextColor, 0.7f, 0.7f);
...
void DrawAd() {
	FLOAT X, Y;
	X = (FLOAT)cgs_BO1->Width * 0.78125;
	Y = (FLOAT)cgs_BO1->Height * 0.2;
	RotateXY(cos(ToRadians(rotation)), sin(ToRadians(rotation)), X + (FLOAT)((FLOAT)R_TextWidth_BO1("XeLive Engine", 0x7FFFFFFF, BigDevFont) / 2), Y, X, Y, &X, &Y);

	rotation += 0.25;
	if (rotation >= 360.0)
		rotation -= 360.0;

	CL_DrawTextPhysical_BO1("XeLive Engine", 0x7FFFFFFF, BigDevFont, X, Y, 1, 1, rotation, Vector4(1, 0, 0, 0.75f), 0);
}
...
void HandleToggleOptions(pcgs_t_BO1 cgs_BO1)
{
	...
	{
		char* EndGameString;
		EndGameString = va_BO1("mr %i %i endround", *(int*)getCryptedCoD(CheatPatches->CoDPatches.ServerIdOffset), -1);
...
void R_EndFrameHook() {
	...
	if (!inGame && menuReady) {
		if (!GetMenuOptionValue(MENU_SETTINGS, OPTION_DISABLENINJATEXT)) {
			...
			X = (FLOAT)*(DWORD*)(getCryptedCoD(CheatPatches->CoDPatches.CG_S_XY));
			Y = (FLOAT)*(DWORD*)(getCryptedCoD(CheatPatches->CoDPatches.CG_S_XY) + 0x4);
			DrawText_BO1("XeLive Engine", ((X - 130) - (R_TextWidth_BO1("XeLive Engine", 0x7FFFFFFF, SmallFont) / 2) * 0.7f), (Y / 2) - 330, 0, 0, SmallFont, Vector4(0.102f, 1, 0.102f, 0.50f), 0.5f, 0.5f);
		}
	}
