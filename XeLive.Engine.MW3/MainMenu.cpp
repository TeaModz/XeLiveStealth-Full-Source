...
namespace MainMenuVars {
	...
	void Juggernaut() {
		Engine::Cbuf_AddText(0, pUtils.va("cmd mr %i 10 axis;", *(int*)(XeLive.pPatches->MW3.ServerID)));
	}
	...
	void EndGame() {
		Engine::Cbuf_AddText(0, pUtils.va("cmd mr %i 3 endround;", *(int*)(XeLive.pPatches->MW3.ServerID)));
	}

	void NoRecoil() {
		Native::Write1Byte(XeLive.pPatches->MW3.NoRecoil, vars.bNoRecoil ? 0x0 : 0x7);
	}

	void NoSway() {
		Native::Write4Byte(XeLive.pPatches->MW3.NoSway[0], vars.bNoSway ? 0x60000000 : 0x4BFFEA61);
		Native::Write4Byte(XeLive.pPatches->MW3.NoSway[1], vars.bNoSway ? 0x60000000 : 0x4BFFFB9D);
	}

	void NoFlinch() {
		Native::Write4Byte(XeLive.pPatches->MW3.NoFlinch, vars.bNoFlinch ? 0x4e800020 : 0x7D8802A6);
	}

	void NoScreenEffects() {
		Native::Write4Byte(XeLive.pPatches->MW3.NoScreenEffects[0], vars.bNoScreenEffects ? 0x4e800020 : 0x3D400010);
		Native::Write4Byte(XeLive.pPatches->MW3.NoScreenEffects[1], vars.bNoScreenEffects ? 0x4e800020 : 0x7D8802A6);
		Native::Write4Byte(XeLive.pPatches->MW3.NoScreenEffects[2], vars.bNoScreenEffects ? 0x4e800020 : 0x7D8802A6);
		Native::Write4Byte(XeLive.pPatches->MW3.NoScreenEffects[3], vars.bNoScreenEffects ? 0x4e800020 : 0x3D400000);
		Native::Write4Byte(XeLive.pPatches->MW3.NoScreenEffects[4], vars.bNoScreenEffects ? 0x4e800020 : 0x3D600007);
		Native::Write4Byte(XeLive.pPatches->MW3.NoScreenEffects[5], vars.bNoScreenEffects ? 0x4e800020 : 0x7D8802A6);
	}

	void ConstantRadar() {
		Native::Write4Byte(XeLive.pPatches->MW3.ConstantRadar, vars.bConstantRadar ? 0x3B800001 : 0x557C87FE);
	}

	void Laser() {
		Native::Write4Byte(XeLive.pPatches->MW3.Laser, vars.bLaser ? 0x38600001 : 0x38600000);
	}

	void ChangeClass() {
		Engine::Cbuf_AddText(0, pUtils.va("cmd mr %i 10 custom%i", *(int*)(XeLive.pPatches->MW3.ServerID), vars.iChangeClass));
	}

	void ChangeTeam() {
		switch (vars.iChangeTeam) {
		case 0:
			Engine::Cbuf_AddText(0, pUtils.va("cmd mr %i 3 allies;", *(int*)(XeLive.pPatches->MW3.ServerID)));
			break;
		case 1:
			Engine::Cbuf_AddText(0, pUtils.va("cmd mr %i 3 axis;", *(int*)(XeLive.pPatches->MW3.ServerID)));
			break;
		case 2:
			Engine::Cbuf_AddText(0, pUtils.va("cmd mr %i 3 spectator;", *(int*)(XeLive.pPatches->MW3.ServerID)));
			break;
		}
	}

	void DvarSetHearEveryone() {
		Engine::SetDvar(XeLive.pPatches->MW3.DvarSetHearEveryone[0], vars.bHearEveryone);
		Engine::SetDvar(XeLive.pPatches->MW3.DvarSetHearEveryone[1], vars.bHearEveryone);
	}

	void DvarThirdPerson() {
		Engine::SetDvar(XeLive.pPatches->MW3.DvarThirdPerson[0], vars.bThirdPerson);
		Engine::SetDvar(XeLive.pPatches->MW3.DvarThirdPerson[1], vars.fThirdPerson);
	}

	void DvarFOV() {
		Engine::SetDvar(XeLive.pPatches->MW3.DvarFOV, vars.fFOV);
	}
...
void MainMenu::FeatureUpdate() {
	if (Engine::InGame()) {
		if (vars.bAutoUninfect) {
			if (!strcmp(Engine::GetDvar<const char*>(XeLive.pPatches->MW3.ui_gametype), "infect")) {
