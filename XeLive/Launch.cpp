...
HRESULT Launch::Initialize() {
	if (!XeLive.bDevkit) {
		if ((GetOptionValueByName = (tLaunchGetOptValByName)Native::ResolveFunction("launch.xex", DL_ORDINALS_GETOPTVALBYNAME)) == NULL) {
			LOG_PRINT("Failed to resolve GetOptionValueByName");
			return S_FAIL;
		}

		if ((SetOptionValueByName = (tLaunchSetOptValByName)Native::ResolveFunction("launch.xex", DL_ORDINALS_SETOPTVALBYNAME)) == NULL) {
			LOG_PRINT("Failed to resolve SetOptionValueByName");
			return S_FAIL;
		}
		...
		if (Utils::FileExists(pLocation)) {
			LOG_PRINT("Parsing launch path for boot xex from %s", pLocation);
			auto ini = IniParse(pLocation);
			if (ini.IsGood()) {
				auto str = ini.ReadString("Paths", "Default", "dash.xex");
				if (str && strlen(str) > 3) {
					string sstr(str);
					...
					strBootXex = sstr;
					LOG_PRINT("Launch path acquired: %s", strBootXex.c_str());
				}
				else {
					strBootXex = "dash.xex";
				}
...
HRESULT Launch::SetLiveBlock(bool bEnabled) {
	...
	// set liveblock
	if (!SetOptionValueByName((char*)"liveblock", &value)) {
		LOG_PRINT("Failed toggling liveblock - %i", value);
		return S_FAIL;
	}

	// set stronglive
	if (!SetOptionValueByName((char*)"livestrong", &value)) {
		LOG_PRINT("Failed toggling livestrong - %i", value);
		return S_FAIL;
	}

	// set xhttp
	if (!SetOptionValueByName((char*)"xhttp", &value)) {
		LOG_PRINT("Failed toggling xhttp - %i", value);
		return S_FAIL;
	}
...
BOOL Launch::IsLiveBlocked() {
	...
	// get liveblock
	if (!GetOptionValueByName((char*)"liveblock", &liveblockValue)) {
		LOG_PRINT("Failed toggling liveblock - %i", liveblockValue);
		return TRUE;
	}

	// get stronglive
	if (!GetOptionValueByName((char*)"livestrong", &liveStrongValue)) {
		LOG_PRINT("Failed toggling livestrong - %i", liveStrongValue);
		return TRUE;
	}
