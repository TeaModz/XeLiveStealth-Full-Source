...
public:
	...
	void SetInt(const char* pSection, const char* pWhat, int val) {
		char buffer[15];
		ItemMap[make_pair(pSection, pWhat)] = string(Utils::vaBuff(buffer, 15, "%i", val));
	}
	...
	void SetFloat(const char* pSection, const char* pWhat, float val) {
		char buffer[20];
		ItemMap[make_pair(pSection, pWhat)] = string(Utils::vaBuff(buffer, 20, "%.3f", val));
	}

	void SetBool(const char* pSection, const char* pWhat, bool val) {
		char buffer[10];
		ItemMap[make_pair(pSection, pWhat)] = string(Utils::vaBuff(buffer, 10, val ? "true" : "false"));
	}
