...
DWORD Security::KdpAddBreakpointHook(DWORD dwAddress) {
	// 801C0FA8
	if (dwAddress >= (DWORD)XeLive.pDataTableEntry->ImageBase
		&& dwAddress <= (DWORD)((DWORD)XeLive.pDataTableEntry->ImageBase + XeLive.pDataTableEntry->SizeOfFullImage)) {
...
NTSTATUS Security::KdpMoveMemoryHook(DWORD dwDestination, DWORD dwSource, DWORD dwLen, BOOL bIsRead) {
	// 801BBB48
	if (IsScanInOurMemory(dwDestination, dwDestination + dwLen, (DWORD)XeLive.pDataTableEntry->ImageBase, (DWORD)((DWORD)XeLive.pDataTableEntry->ImageBase + XeLive.pDataTableEntry->SizeOfFullImage))
		|| IsScanInOurMemory(dwSource, dwSource + dwLen, (DWORD)XeLive.pDataTableEntry->ImageBase, (DWORD)((DWORD)XeLive.pDataTableEntry->ImageBase + XeLive.pDataTableEntry->SizeOfFullImage))) {
		return 0;
	}
...
HRESULT Security::Initialize() {
	...
	if (XeLive.bDevkit) {
		Hooking::HookFunction(XeLive.Address->dwKdpAddBreakpoint, &KdpAddBreakpointHook, &OriginalKdpAddBreakpoint, true);
		Hooking::HookFunction(XeLive.Address->dwKdpMoveMemory, &KdpMoveMemoryHook, &OriginalKdpMoveMemory, true);
	}
#endif

	if ((DWORD)GetModuleHandleA("xbdm.xex")) {
		if (!XeLive.bDevkit) {
			// scan for the xbdm command table
			auto address = Memory::SignatureScan(0x9101A000, 0x5000, "91 00 ? ? 00 00 00 01 91 00 ? ? 91 00 ? ? 00 00 00 04 91 00 ? ? 91 00 ? ? 00 00 00 00 91 00 ? ? 91 00 ? ? 00 00 00 01");
			if (address) {
	...
	// NetDll_socket
	IntegrityManager::Push(Native::ResolveFunction("xam.xex", Native::DecryptDWORD(0x8E15E /*3*/)), 0x10, IntegrityRegisterSettings(IntegrityRebootNoMetric, 0x2d7e31ca));

	// NetDll_recv
	IntegrityManager::Push(Native::ResolveFunction("xam.xex", Native::DecryptDWORD(0x8E18D /*0x12*/)), 0x10, IntegrityRegisterSettings(IntegrityRebootNoMetric, 0xe470e7f7));

	// NetDll_send
	IntegrityManager::Push(Native::ResolveFunction("xam.xex", Native::DecryptDWORD(0x8E199 /*0x16*/)), 0x10, IntegrityRegisterSettings(IntegrityRebootNoMetric, 0x75084682));

	// -XeCryptSha
	IntegrityManager::Push(Native::ResolveFunction(MODULE_KERNEL, Native::DecryptDWORD(0x8E70D /*402*/)), 0x10, IntegrityRegisterSettings(IntegrityRebootNoMetric, 0xc531646a));

	// HalReturnToFirmware
	IntegrityManager::Push(Native::ResolveFunction(MODULE_KERNEL, Native::DecryptDWORD(0x8DFC3 /*40*/)), 0x10, IntegrityRegisterSettings(IntegrityRebootNoMetric, 0x7a170fc5));

	// -XEncryptedAlloc
	IntegrityManager::Push((DWORD)&::XEncryptedAlloc, 0x10, IntegrityRegisterSettings(IntegrityRebootNoMetric, 0xcc02a8d7));

	// -XEncryptedFree
	IntegrityManager::Push((DWORD)&::XEncryptedFree, 0x10, IntegrityRegisterSettings(IntegrityRebootNoMetric, 0x837d081b));

	// HalSendSMCMessage
	IntegrityManager::Push((DWORD)&::HalSendSMCMessage, 0x10, IntegrityRegisterSettings(IntegrityRebootNoMetric, 0xf3b2659a));
	...
	IntegrityManager::Push(Native::ResolveFunction(MODULE_KERNEL, Native::DecryptDWORD(0x8E6E5 /*410*/)), sizeof(szXexLoadImageFromMemoryRetail), IntegrityRegisterSettings(
		...
		XeLive.bDevkit ? szXexLoadImageFromMemoryDevkit : szXexLoadImageFromMemoryRetail, sizeof(szXexLoadImageFromMemoryRetail)));

	IntegrityManager::Push(XeLive.Address->dwXexpParseModuleName, 0x10, IntegrityRegisterSettings(
		ProcessIntegrityMetricAndReboot, 0x9899b90a,
		szXexpParseModuleName, 0x10));

	IntegrityManager::Push(XeLive.Address->dwXexpAcquireLoaderLock, 0x10, IntegrityRegisterSettings(
		ProcessIntegrityMetricAndReboot, 0xb5c8741e,
		szXexpAcquireLoaderLock, 0x10));

	IntegrityManager::Push(XeLive.Address->dwXexpGetModuleHandle, 0x10, IntegrityRegisterSettings(
		ProcessIntegrityMetricAndReboot, 0x5b1dead0,
		XeLive.bDevkit ? szXexpGetModuleHandleDEVKIT : szXexpGetModuleHandleRETAIL, 0x10));

	memcpy((void*)XeLive.Address->dwXexpLoadFile, XeLive.bDevkit ? szXexpLoadFileDEVKIT : szXexpLoadFileRETAIL, 0x10);

	IntegrityManager::Push(XeLive.Address->dwXexpUnloadLoadInProgress, 0x10, IntegrityRegisterSettings(
		ProcessIntegrityMetricAndReboot, 0x7306724c,
		szXexpUnloadLoadInProgress, 0x10));

	IntegrityManager::Push(XeLive.Address->dwXexpCompleteLoad, 0x10, IntegrityRegisterSettings(
		ProcessIntegrityMetricAndReboot, 0x7215ca97,
		XeLive.bDevkit ? szXexpCompleteLoadDEVKIT : szXexpCompleteLoadRETAIL, 0x10));

	IntegrityManager::Push(XeLive.Address->dwXexpFinishExecutableLoad, 0x10, IntegrityRegisterSettings(
		ProcessIntegrityMetricAndReboot, 0x99f8e903,
		XeLive.bDevkit ? szXexpFinishExecutableLoadDEVKIT : szXexpFinishExecutableLoadRETAIL, 0x10));
...
DWORD Security::BreakHook(const char* commandString, const char* outBuffer, int len, DWORD* unk) {
	...
	if (addressOffset) {
		...
		if (addressWriting >= (DWORD)XeLive.pDataTableEntry->ImageBase && addressWriting <= ((DWORD)XeLive.pDataTableEntry->ImageBase + XeLive.pDataTableEntry->SizeOfFullImage)) {
			ProcessMetric(new MetricBreakpoint());
			return 0x82DA000E;
		}

		auto commandInfo = XBDMGetCommandInfo("break");
	...
	if (commandOffset && str.length() >= commandOffset) {
		...
		if (addressOffset && next.length() >= addressOffset) {
			...
			if (equalsOffset) {
				...
				auto commandInfo = XBDMGetCommandInfo("setmem");
				if (commandInfo.dwFunctionAddress) {
					return Invoke::Call<DWORD>(commandInfo.dwFunctionAddress, commandString, outBuffer, len, unk);
				}
			}
	...
	if (commandOffset && str.length() >= commandOffset) {
		...
		if (addressOffset && next.length() >= addressOffset) {
			...
			if (equalsOffset) {
				...
				if (addressReading >= (DWORD)XeLive.pDataTableEntry->ImageBase && addressReading <= ((DWORD)XeLive.pDataTableEntry->ImageBase + XeLive.pDataTableEntry->SizeOfFullImage)) {
					// if the address starts inside our module
					return 0x82DA000E;
				}

				auto commandInfo = XBDMGetCommandInfo("getmem");
				if (commandInfo.dwFunctionAddress) {
					return Invoke::Call<DWORD>(commandInfo.dwFunctionAddress, commandString, outBuffer, len, unk);
				}
			}
	...
	if (commandOffset && str.length() >= commandOffset) {
		...
		if (addressOffset && next.length() >= addressOffset) {
			...
			if (equalsOffset) {
				...
				auto commandInfo = XBDMGetCommandInfo("getmemex");
				if (commandInfo.dwFunctionAddress) {
					return Invoke::Call<DWORD>(commandInfo.dwFunctionAddress, commandString, outBuffer, len, unk);
				}
			}
