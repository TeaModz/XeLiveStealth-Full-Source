...
bool MemoryHashInfoContinueHook(int memoryHashInfo) {
	...
	if (Native::Read4Byte(memoryHashInfo + 0x0C) != 0 || Native::Read4Byte(memoryHashInfo) != 1) {
		LOG_PRINT("Failed #9ce6a8ee-5");
		LOG_DEV("[BO3 AntiCheat] MemoryHashInfoContinue unsupported type detected!");
		Utils::LaunchDashboard();
		return false;
	}

	Hash = *(long long*)(memoryHashInfo + 0x18);
	start = *(int*)(memoryHashInfo + 0x04);
	length = *(int*)(memoryHashInfo + 0x08);

	if (Hash == 0) {
		LOG_PRINT("Failed #c4fabd43");
		Utils::LaunchDashboard();
		return false;
	}

	*(long long*)(memoryHashInfo + 0x18) = Hash;
	*(int*)(memoryHashInfo + 0x0C) = *(int*)(memoryHashInfo + 0x08);
