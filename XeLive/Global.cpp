...
HRESULT Global::Initialize() {
	XeLive.dwUpdateSequence = 9;

	// allocates the stealth patch ptr
	XeLive.Address = (Addresses*)Native::XPhysicalAlloc(sizeof(Addresses), MAXULONG_PTR, NULL, PAGE_READWRITE);

	// sets the hardcoded addresses
	LOG_PRINT("Initializing #5a67b09a");
	XeLive.Address->Apply(XeLive.bDevkit);

	DWORD* pPatches;
	DWORD dwPatchSize;

	if (!XGetModuleSection(XeLive.hMyModule, "PP", (void**)&pPatches, &dwPatchSize)) {
		LOG_PRINT("Couldn't find #256af0d7");
		return S_FAIL;
	}
	...
	if (PluginData.size() > 0) {
		for (int i = 0; i < PluginData.size(); i++) {
			...
			if (data.dwTitle == dwTitle) {
				LOG_DEV("Found plugin for title 0x%X", dwTitle);
				if (!data.bEnabled) {
					LOG_DEV("...Aborting, plugin is not enabled for client");
					return FALSE;
				}

				if (data.dwTitleTimestamp == dwTimestamp) {
					LOG_DEV("...Timestamp match, successful plugin able to be loaded");
					if (pOut) *pOut = PluginData[i];
					return TRUE;
				}

				LOG_DEV("...Aborting, timestamp doesn't match. Server: %X, Client: %X", data.dwTitleTimestamp, dwTimestamp);
				return FALSE;
			}
		}
	}
