...
LONG WINAPI Handler(struct _EXCEPTION_POINTERS* lpExceptionInfo) {
	...
	rename(Utils::sprintfaA("%s%s", "XELIVE:\\", "Crashlog").c_str(), NewLogPath.c_str());
	...
	XShowMessageBoxUI(XUSER_INDEX_ANY, L"F3 CRASH", L"Freestyle 3 crashed, what would you like to do?", 3, btnOptions, 0, XMB_ERRORICON, &mbresult, &overlapped);
