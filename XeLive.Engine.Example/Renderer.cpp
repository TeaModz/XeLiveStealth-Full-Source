...
void Renderer::DrawLine(float X1, float Y1, float X2, float Y2, float width, Color color, Material* Shader) {
	...
	Engine::CG_DrawRotatedPicPhysical(XeLive.pPatches->MW3.ScreenPlacement, X, Y, L2, width, Angle, color.GetFloatPtr(), Shader);
}
...
void Renderer::Render() {
	DrawRect(0, 0, 1280.0f, 60, eJustify::JustifyLeft, Color(0x2424240FF)); //Nav Background
	for (int i = 0; i < 6; i++) {
		DrawOutlinedRect(0, 0, 0 + (i * 256.0f), 60, 2, Color(0x70e000FF), Color(0x70e000FF), true);
	}
